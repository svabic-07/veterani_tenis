-- =====================================================================
-- TVS · 0029 · finish_tournament v5 — popravke iz Codex+Claude revizije
-- #4: turnir SAMO sa solo kategorijama može da se završi (validacija
--     prihvata i solo-kvalifikovane kategorije, ne samo žrebove)
-- #5: solo pravilo važi samo kad kategorija NIKAD nije imala žreb
--     (opozvan žreb = otkazana kategorija, ne solo pobednik)
-- #7: solo pravilo samo za SINGL (dubl/miks čeka logiku parova)
-- #8: čista grupa — tie-break po spec-u: pobede → međusobni duel (za
--     dvoje izjednačenih) → set-razlika → gem-razlika → determinizam;
--     do sada je izjednačenje lomio UUID
-- =====================================================================
...
comment on function public.finish_tournament is
  '„ZAVRŠI TURNIR“: validira žrebove i tablicu, obračunava bodove (gost bez bodova — prenos niz kostur; jedini prijavljeni u kategoriji = pobednik bez borbe), upisuje ranking_points i nedeljni rang. Staff ili direktor.';
