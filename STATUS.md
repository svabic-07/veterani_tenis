...
**Gap prema starom sajtu — ZATVOREN 2026-07-19 (migracija 32 + UI):**
- **Pravilnik**: nove sekcije 06 Brisanje bodova · 07 Posebne odredbe (predaja, gosti, solo, sankcije, polufinala) · 08 FAQ (7 pitanja, prilagođeno novom sistemu).
- **Rang-liste po starosnim kategorijama** (20–75+) — drugi red čipova na `/rang-liste` (podaci su već postojali).
- **Učlanjenje** (`/uclanjenje`): javna forma → `membership_requests` (RLS public insert) → koordinator odobrava u panelu (`resolve_membership_request` kreira igrača + kontakt, klub po nazivu, audit); linkovi u footeru i na `/prijava`.
- **Sankcije v1**: tabela `sanctions` (opomena/oduzimanje bodova/suspenzija, važi-do) — UI u `/koordinator/clanovi` (po igraču, sklopivo); **suspenzija blokira samoprijavu** (can_self_enter_event v3).
- **Galerija**: storage bucket `galerija` (javno čitanje, staff upload ≤8MB JPG/PNG/WebP) + `gallery_photos`; javna `/galerija` (ISR 5 min) + `/koordinator/galerija` (upload iz browsera, brisanje); link u footeru.
- **Paginacija direktorijuma** (60 po strani, prethodna/sledeća) · **kalendar filter po seriji** (čipovi, kombinuje se sa godinom arhive) · **H2H na profilu** (protivnici sa najviše mečeva, pobede:porazi).
